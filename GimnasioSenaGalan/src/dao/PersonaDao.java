	...
	public String editarPersona(UsuarioVo UsuarioVo) {
		...
		try {
			String consulta = "update registrados set nombre=?, apellido=?, ficha=?, telefono=?, correo=?,"
					+ "idRutina=?,formacion=?, sexo=? where documento="+UsuarioVo.getDocumento();
			...
			preparedStatement.setString(6, UsuarioVo.getRutina());
			preparedStatement.setString(7, UsuarioVo.getFormacion());
			preparedStatement.setString(8, UsuarioVo.getSexo());			
		...
		try {
			if (connection != null) {
				...
				if (result.next() == true) {
					...
					miUsuario.setRutina(result.getString("rutina"));
					miUsuario.setRutina(result.getString("rutina"));
					miUsuario.setRutina(result.getString("rutina"));
					miUsuario.setRutina(result.getString("rutina"));
					miUsuario.setRutina(result.getString("rutina"));
